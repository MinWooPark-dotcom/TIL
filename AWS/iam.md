...
* A 계정에서 B 계정의 S3 버킷에 접근하도록 구성
* IAM Role Assume을 통해 교차 계정 권한 위임 구현

## 실습 절차 요약
1. B 계정: IAM Role 생성 및 Trust Policy 설정
2. B 계정: S3 버킷 생성 및 접근 권한 정책 구성
3. A 계정: IAM 사용자에게 AssumeRole 권한 부여
4. A 계정 CLI: `sts:AssumeRole` 실행 후 임시 자격 증명 획득
5. A 계정: 임시 자격 증명을 환경 변수로 설정 후 S3 접근 확인

### 아키텍처 흐름
```
[A 계정 IAM 사용자] 
      ↓ sts:AssumeRole
[B 계정 IAM Role: CrossAccountS3AccessRole]
      ↓ 임시 자격 증명 발급
...
### B 계정: IAM Role Trust Policy: 누가 이 역할을 맡을 수 있는가?
...
### B 계정: Role에 부여할 권한 정책: 이 역할이 맡으면 할 수 있는 권한
...
- Trusted Entity Type을 AWS Account 선택 후 Account ID에 A 계정의 AWS Account ID를 입력 후 생성 버튼 클릭
- Role 생성 후 Trsut relationships 확인하여 Principal.AWS의 값에 A 계정 AWS Account ID가 잘 입력 되었는지, Action은 "sts:AssumeRole"이 설정되었는지 확인
...
3. A 계정에서 AWS CLI로 AssumeRole 설정
